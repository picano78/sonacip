...
def _auto_upgrade_db(app: Flask) -> None:
    ...
    def _try_alembic_upgrade():
        try:
            ...
            # Never crash startup if migrations cannot run.
            try:
...
def create_app(config_name: str | None = None) -> Flask:
    ...
    # SQLite is the default DB (fresh VPS installs), PostgreSQL only if DATABASE_URL is set.
    if not app.config.get("TESTING"):
        ...
        default_sqlite_path = os.path.join(uploads_dir, "sonacip.db")
        db_uri = os.getenv("DATABASE_URL", f"sqlite:///{default_sqlite_path}")
