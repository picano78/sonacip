#!/bin/bash
# SONACIP Production Installer - Ubuntu 24.04 Compatible
...
# Logging
LOG_FILE="/var/log/sonacip_production_install.log"
...
print_header "SONACIP Production Installer - Ubuntu 24.04"
echo ""
echo "🚀 Installing SONACIP Production on Ubuntu 24.04"
...
print_step() {
    ...
    echo -e "${BLUE}[Step $step_num/12]${NC} ${BOLD}$step_desc${NC} (${elapsed}s elapsed)" | tee -a "$LOG_FILE"
}

print_step "1/12" "Detecting system requirements"
if [[ $EUID -ne 0 ]]; then
    print_error "This script must be run as root"
    print_status "Use: sudo bash ionos_production_installer.sh"
    exit 1
fi
...
print_step "2/12" "Setting up 2GB swap file for low memory VPS"
if [[ $TOTAL_MEM -lt 1024 ]]; then
    if [[ ! -f /swapfile ]]; then
...
print_step "3/12" "Updating system packages"
...
print_step "4/12" "Installing Python and development tools"
PYTHON_PACKAGES=(
    "python3-full"
    "python3-venv"
    "python3-pip"
...
print_step "5/12" "Verifying Python installation"
...
print_step "6/12" "Creating service user"
...
print_step "7/12" "Copying project files (preserving structure)"
...
print_step "8/12" "Creating Python virtual environment"
cd "$INSTALL_DIR"
if [[ ! -d "venv" ]]; then
...
print_step "9/12" "Installing Python dependencies from requirements.txt"
cd "$INSTALL_DIR"
...
print_step "10/12" "Detecting application entry point"
...
print_step "11/12" "Creating environment configuration"
if [[ ! -f "$INSTALL_DIR/.env" ]]; then
    print_status "Creating .env file..."
    cat > "$INSTALL_DIR/.env" << EOF
# SONACIP Production Configuration - Generated by Production Installer
# Generated on: $(date)
...
print_header "Production Installation Complete!"
echo ""
print_success "🎉 SONACIP has been successfully installed in production!"
echo ""
