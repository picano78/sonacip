...
def main() -> int:
    ...
    with app.app_context():
        ...
        try:
            ...
            # Mark run failed if we had an unexpected crash mid-run
            try:
                if run_row:
                ...
                run_row.summary = json.dumps(
                    {
                        ...
                        "compliance_status_updated": compliance_updated,
                        "compliance_reminders_sent": compliance_sent,
                        "health_snapshots_created": health_created,
                        "ads_events_deleted": ads_events_deleted,
                    }
                )
                db.session.add(run_row)
                db.session.commit()
